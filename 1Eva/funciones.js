...
export function listaPrestamos() {
    ...
    tr.innerHTML = "<tr><td>id</td></tr><tr><td>titulo</td></tr><tr><td>autor</td></tr><tr><td>editorial</td></tr><tr><td>tema</td></tr><tr><td>prestado</td></tr>";
    tabla.appendChild(tr);

    arrayPrestamos.forEach(element => {
        
        let trw = document.createElement('tr');
        trw.innerHTML = `<th><td>${element.id}</td></th><th><td>${element.titulo}</td></th><th><td>${element.autor}</td></th><th><td>${element.editorial}</td></th><th><td>${element.tema}</td></th><th><td>${element.prestado}</td></th>`;
...
export function imgPrestamos() {
    ...
    img.alt = "Imagen de prestamo de biblioteca";
...
export function listaSocios() {
    ...
    tr.innerHTML = "<tr><td>idSocio</td></tr><tr><td>nombre</td></tr><tr><td>mail</td></tr>";
    tabla.appendChild(tr);

    arraySocios.forEach(element => {
        
        let trw = document.createElement('tr');
        trw.innerHTML = `<th><td>${element.idSocio}</td></th><th><td>${element.nombre}</td></th><th><td>${element.mail}</td></th>`;
        tabla.appendChild(trw);
    });
...
export function imgSocios() {
    ...
    img2.src = 'userbiblio.jpg';
    img2.alt = "Imagen de usuario de biblioteca";
    img2.style="width: 500px; height: 500px";
    return img2;
}
